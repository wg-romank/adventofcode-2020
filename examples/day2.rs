...
fn policy_maker(policy: &str, str: &str) -> Option<(bool, bool)> {
    ...
    let count = str.chars().fold(
        0,
        |acc, next| if next == letter_char { acc + 1 } else { acc },
    );

    let rule1 = times_low_i <= count && count <= times_high_i;
    let rule2 = (str.chars().nth(times_low_i)? == letter_char)
...
fn is_valid_password(password_policy: &str) -> Option<(bool, bool)> {
    let (policy, password) = password_policy.split(':').next_tuple()?;

    policy_maker(policy, password)
}

fn main() {
    let inputs = std::fs::read_to_string("inputs/input2").unwrap();

    let parsed_inputs = inputs.split('\n').collect::<Vec<&str>>();

    let (count1, count2) = parsed_inputs.into_iter().flat_map(is_valid_password).fold(
        (0, 0),
        |(acc_a, acc_b), (a, b)| match (a, b) {
